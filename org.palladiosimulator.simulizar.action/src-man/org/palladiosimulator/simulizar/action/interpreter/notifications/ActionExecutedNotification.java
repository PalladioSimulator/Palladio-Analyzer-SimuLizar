package org.palladiosimulator.simulizar.action.interpreter.notifications;

import java.util.Objects;

import org.eclipse.emf.common.notify.impl.NotificationImpl;
import org.palladiosimulator.simulizar.action.core.Action;

/**
 * Notification class to indicate that an {@link Action} was executed successfully.
 */
public class ActionExecutedNotification extends NotificationImpl {
    public static final int ACTION_EXECUTED_EVENT_TYPE = 42;
    
    /**
     * Initializes a new instance of the {@link ActionExecutedNotification} class.
     * @param executedAction The {@link Action} that terminated successfully.
     * @throws NullPointerException In case {@code executedAction} is {@code null}.
     */
    public ActionExecutedNotification(Action executedAction) {
    	super(ACTION_EXECUTED_EVENT_TYPE, null, Objects.requireNonNull(executedAction, "The given action must not be null"));
    }
    
    /**
     * Gets action that was executed.<br>
     * <b>Note:</b> Calling this method is is equivalent to calling {@link #getNotifier()}.
     * @return The {@code Action} that was executed.
     * @see #getNotifier()
     */
    @Override
    public Action getNewValue() {
        //typesafe as passed in sole ctor
        return (Action) super.newValue;
    }
    
    /**
     * Gets action that is associated with this notification.<br>
     * <b>Note:</b> Calling this method is is equivalent to calling {@link #getNewValue()}.
     * @return The {@code Action} that was executed.
     * @see #getNewValue()()
     */
    @Override
    public Action getNotifier() {
        return this.getNewValue();
    }
    
    /**
     * {@inheritDoc}
     * @return This method always returns {@code true}.
     */
    @Override
    public boolean isTouch() {
        return true;
    }
    
    /**
     * {@inheritDoc}
     * @return This method always returns {@code false}.
     */
    @Override
    public boolean isReset() {
        return false;
        
    }
}
