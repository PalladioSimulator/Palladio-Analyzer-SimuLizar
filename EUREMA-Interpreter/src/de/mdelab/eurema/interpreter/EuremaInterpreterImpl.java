...
public class EuremaInterpreterImpl implements EuremaInterpreter {
	...
	private static Logger logger = LogManager
			.getLogger(EuremaInterpreterImpl.class);
	...
	private void setUpInterpreter() {
		...
		this.rs.getResourceFactoryRegistry().getExtensionToFactoryMap()
				.put("eurema", new XMIResourceFactoryImpl());
		this.rs.getResourceFactoryRegistry().getExtensionToFactoryMap()
		...
		this.rs.getPackageRegistry().put(euremaPackage.getNsURI(),
				euremaPackage);
		Metamodel.INSTANCE.initialize(euremaPackage);
		logger.debug("  |_ Loaded the EUREMA metamodel: namespace URI is "
				+ euremaPackage.getNsURI());

		// create EUREMA event type hierarchy
		logger.debug(" |_ Create the EUREMA event type hierarchy...");
		eurema.EventType rootEventType = EuremaEventTypeHierarchy.INSTANCE
				.initialize();
		Resource eEventTypeHierarchy = this.rs.createResource(URI
				.createURI(this.euremaProps.getEventTypeHierarchyURI()));
		...
		eurema.RuntimeEnvironment eRuntimeEnvironment = Metamodel.INSTANCE
				.getEuremaFactory().createRuntimeEnvironment();
		Resource eRuntimeEnvironmentResource = this.rs.createResource(URI
				.createURI(this.euremaProps.getRuntimeEnvironmentURI()));
	...
	public EventQueue execute(eurema.Architecture eArchitecture) {
		if (eArchitecture == null) {
			throw new EuremaInterpreterException(
					"The architecture to be executed is null.");
		}

		// setting up the model repository
		logger.debug(" |_ Setting up the EUREMA model repository...");
		ModelRepository.INSTANCE.initialize(this.rs,
				eArchitecture.getModelResourceSet());
	...
	public EventQueue execute(String euremaArchitectureModelUri) {
		if (euremaArchitectureModelUri == null
				|| euremaArchitectureModelUri.equals("")) {
			throw new EuremaInterpreterException(
					"The URI of the architecture to be executed is null or the empty String.");
		}
		eurema.Architecture eArchitecture = this
				.loadArchitecture(euremaArchitectureModelUri);
	 ...
	 *         instance.
	 */
	private eurema.Architecture loadArchitecture(
			String euremaArchitectureModelUri) {
		if (euremaArchitectureModelUri == null
				|| euremaArchitectureModelUri.equals("")) {
			throw new EuremaInterpreterException(
					"The URI of the architecture to be executed is null or the empty String.");
		}
		logger.debug("Loading the EUREMA model from the URI "
				+ euremaArchitectureModelUri);
		...
		if (contents.size() == 0) {
			throw new EuremaInterpreterException(
					"EUREMA architecture model contains no root object.");
		}
		if (contents.size() != 1) {
			throw new EuremaInterpreterException(
					"EUREMA architecture model contains more than one root objects.");
		}
		EObject rootObject = contents.get(0);
		if (rootObject == null) {
			throw new EuremaInterpreterException(
					"EUREMA architecture model contains a null root object.");
		}
		if (!(rootObject instanceof eurema.Architecture)) {
			...
			logger.debug("EUREMA model successfully loaded from the URI "
					+ euremaArchitectureModelUri);
