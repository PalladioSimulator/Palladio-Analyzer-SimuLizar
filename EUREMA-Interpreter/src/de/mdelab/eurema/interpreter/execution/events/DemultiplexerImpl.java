...
public class DemultiplexerImpl implements Demultiplexer {
	...
	private static Logger logger = LogManager
			.getLogger(DemultiplexerImpl.class);
	 ...
	 */
	private Map<eurema.EventType, List<EventDrivenModuleScheduler>> eventSubscriptions;
	 ...
	 */
	private List<eurema.Event> allSensorEvents;
	 ...
	 */
	public DemultiplexerImpl(
			EventQueueInternal globalSensorEventQueue,
	...
	public void run() {
		while (!this.shutdown) {
			try {
				...
				List<EventDrivenModuleScheduler> subscribers = this.eventSubscriptions
						.get(eEventType);
				String msg = "Demultiplexing of sensor event "
						+ EuremaEventTypeHierarchy.INSTANCE
								.getEEventAsString(eEvent) + " to "
						+ subscribers.size() + " subscribing modules: ";
