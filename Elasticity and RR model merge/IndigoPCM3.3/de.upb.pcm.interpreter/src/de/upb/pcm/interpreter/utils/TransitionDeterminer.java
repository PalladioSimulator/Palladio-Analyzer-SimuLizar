...
public class TransitionDeterminer
{
    ...
    * 
    * @param condition the condition (must be a boolean expression).
    ...
    * of probabilities is 0.4, the corresponding value in the summed probability list is 0.4+0.3 and
    * so on.
    * 
    ...
    * probabilities.
    * 
    * @param branchTransitions the list of branch transition.
   ...
   {
      final List<Double> summedProbabilityList = createSummedProbabilityList(extractProbabiltiesUsageModel(branchTransitions));

      final int transitionIndex = getRandomIndex(summedProbabilityList, config);
    ...
    * 
    * @param guardedBranchTransitions the list of guarded branch transition.
         ...
         final EList<AbstractBranchTransition> guardedBranchTransitions)
   {
       ...
       * forbids the use of INNER characterizations in branch conditions. Thus, this problem has not
       * to be addressed like in the collection iterator (EvaluationProxies and the same value for
       * all occurrences in one iteration).
       */
      int i = 0;
   ...
   {
      final List<Double> summedProbabilityList = createSummedProbabilityList(extractProbabiltiesRDSEFF(probabilisticBranchTransitions));
   ...
   public AbstractBranchTransition determineTransition(final EList<AbstractBranchTransition> abstractBranchTransitions)
   {
      ...
      if (abstractBranchTransitions.get(0) instanceof ProbabilisticBranchTransition)
      {
         ...
         branchTransition = determineProbabilisticBranchTransition(abstractBranchTransitions);

      }
      else
      {
         InterpreterLogger.debug(logger, "Found GuardedBranchTransitions");
         branchTransition = determineGuardedBranchTransition(abstractBranchTransitions);
      }
      return branchTransition;
   }
    ...
    * 
    * @param probabilisticBranchTransitions the list of ProbabilisticBranchTransition.
    * @return a list only containing the probabilities.
    */
   protected List<Double> extractProbabiltiesRDSEFF(final EList<AbstractBranchTransition> probabilisticBranchTransitions)
   {
    ...
    * 
    * @param branchTransitions the list of BranchTransition.
    * @return a list only containing the probabilities.
    */
    ...
    * 
    * @param summedProbabilityList a list of summed probabilities.
    * @param simuComConfig the SimuCom config.
