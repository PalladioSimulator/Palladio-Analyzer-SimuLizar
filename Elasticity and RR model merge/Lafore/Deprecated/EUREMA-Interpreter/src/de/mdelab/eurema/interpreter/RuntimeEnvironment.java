...
class RuntimeEnvironment {
	...
	private static Logger logger = LogManager
			.getLogger(RuntimeEnvironment.class);

	/**
	 * The {@code eurema.RuntimeEnvironment} instance in the EUREMA model.
	 */
	private eurema.RuntimeEnvironment eRuntimeEnvironment;
	...
	EventQueue execute(eurema.Architecture eArchitecture) {
		logger.debug("Preparing the execution of the architecture defined in the EUREMA model "
				+ eArchitecture);
		...
		EventQueue queue =
				this.executionManager.initialize(this.eRuntimeEnvironment);
