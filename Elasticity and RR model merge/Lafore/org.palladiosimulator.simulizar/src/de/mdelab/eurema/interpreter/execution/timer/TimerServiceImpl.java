...
public class TimerServiceImpl implements TimerService {
	...
	public void createTimer(int periodInSec, EventQueueInternal timerEventQueue) {
		...
		logger.debug("Created timer expiring in " + periodInSec + " seconds to send a timer event to the queue "
	...
	private class SendTimerEvent extends TimerTask {
		 ...
		 */
		private final EventQueueInternal timerEventQueue;
		 ...
		 */
		private SendTimerEvent(int periodInSec, EventQueueInternal timerEventQueue) {
		...
		@Override
		public void run() {
			eurema.Event timerEvent = Metamodel.INSTANCE.getEuremaFactory().createEvent();
			timerEvent.setName("Period of " + this.periodInSec + " seconds has elapsed.");
			eurema.EventType timerEventType = EuremaEventTypeHierarchy.INSTANCE.getEEventType(EuremaEventType.TIMER);
			timerEvent.setType(timerEventType);
			this.timerEventQueue.add(timerEvent);
			logger.debug("A timer event notifying about the expiration of " + this.periodInSec
					+ " seconds has been sent to the queue " + this.timerEventQueue.getName());
		}
	}
