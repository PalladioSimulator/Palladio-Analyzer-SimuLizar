...
public class SDExecutor {
    ...
    */
     ...
     * 
     */
	private static final EClass DE_MODEL_ECLASS = DlimPackage.eINSTANCE
			.getSequence();
    ...
    */
     ...
     * 
     */
	private static final String DE_MODEL = "environmentModel";
	 ...
	 *            the model access factory used to access the SD, PCM@runtime
	 *            and PRM models.
	 */
	public SDExecutor(final IModelAccess modelAccessFactory) {
		super();
		this.storyDiagrams = modelAccessFactory.getStoryDiagrams();
		this.globalPcmResourceSetPartition = modelAccessFactory
				.getGlobalPCMModel();
		this.prmModel = modelAccessFactory.getPRMModel();
		this.demModel = modelAccessFactory.getDEModel();
		try {
			this.sdmInterpreter = new StoryDrivenEclipseInterpreter(this
					.getClass().getClassLoader());
		} catch (final SDMException e) {
			throw new RuntimeException(
					"Unable to inialise SD interpreter engine", e);
		}
	...
	private Collection<Activity> createBindingsForActivities() {
		...
		for (final Activity activity : ActivitiesFromModels) {
			final Activity activityWithBindings = ActivityLoader
					.createBindings(activity, new String[] { USAGE_MODEL,
							DE_MODEL, SYSTEM_MODEL, REPOSITORY_MODEL,
							ALLOCATION_MODEL, RESOURCE_ENVIRONMENT_MODEL,
							PRM_MODEL, MONITORED_ELEMENT }, new EClassifier[] {
							USAGE_MODEL_ECLASS, DE_MODEL_ECLASS,
							SYSTEM_MODEL_ECLASS, REPOSITORY_MODEL_ECLASS,
							ALLOCATION_MODEL_ECLASS,
	 ...
	 * 
	 * @return List<Variable<EClassifier>> list of variables and their according
	 *         EClassifiers
	 */
	private List<Variable<EClassifier>> createParameter() {
		...
		final Variable<EClassifier> usageModelParameter = new Variable<EClassifier>(
				USAGE_MODEL, USAGE_MODEL_ECLASS,
				this.globalPcmResourceSetPartition.getUsageModel());
		final Variable<EClassifier> environmentModelParameter = new Variable<EClassifier>(
				DE_MODEL, DE_MODEL_ECLASS, this.demModel);
		final Variable<EClassifier> systemModelParameter = new Variable<EClassifier>(
				SYSTEM_MODEL, SYSTEM_MODEL_ECLASS,
				this.globalPcmResourceSetPartition.getAllocation()
		...
		final Variable<EClassifier> allocationModelParameter = new Variable<EClassifier>(
				ALLOCATION_MODEL, ALLOCATION_MODEL_ECLASS,
				this.globalPcmResourceSetPartition.getAllocation());
		...
		parameters.add(environmentModelParameter);
