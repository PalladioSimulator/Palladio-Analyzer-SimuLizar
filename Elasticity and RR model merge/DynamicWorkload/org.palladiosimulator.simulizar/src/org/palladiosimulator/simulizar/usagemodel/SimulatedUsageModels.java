...
public class SimulatedUsageModels {
	...
	private final EList<UsageScenario> usageScenarios;

	public SimulatedUsageModels(final InterpreterDefaultContext rootContext) {
		super();
		this.rootContext = rootContext;
		this.usageScenarios = rootContext.getModelAccess().getGlobalPCMModel().getUsageModel().getUsageScenario_UsageModel();
	}
	...
	public IWorkloadDriver[] getWorkloadDrivers() {
		// final EList<UsageScenario> usageScenarios =
		// rootContext.getModelAccess().getGlobalPCMModel().getUsageModel().getUsageScenario_UsageModel();
		initializeWorkloadsForUsageScenarios();
	...
	private void initializeWorkloadsForUsageScenarios() {
		for (UsageScenario us : this.usageScenarios) {
			if (us.getWorkload_UsageScenario() == null) {
				...
				pcmRandomVariable.setOpenWorkload_PCMRandomVariable(openWorkLoad);
				pcmRandomVariable.setSpecification("Exp(0.01)");
				openWorkLoad.setInterArrivalTime_OpenWorkload(pcmRandomVariable);
				us.setWorkload_UsageScenario(openWorkLoad);
				LOGGER.debug("No Workload found, creating one.");
			}
		}
	}
	...
	private IWorkloadDriver getWorkloadDriver(final UsageScenario usageScenario) {
		...
		if (workload.eClass() == UsagemodelPackage.eINSTANCE.getClosedWorkload()) {
			final de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload driver = getClosedWorkloadDriver(workload, usageScenario);
			closedWorkloads.put((ClosedWorkload) workload, driver);
			return driver;
		} else if (workload.eClass() == UsagemodelPackage.eINSTANCE.getOpenWorkload()) {
			final de.uka.ipd.sdq.simucomframework.usage.OpenWorkload driver = getOpenWorkloadDriver(workload, usageScenario);
	...
	private de.uka.ipd.sdq.simucomframework.usage.ClosedWorkload getClosedWorkloadDriver(final Workload workload, final UsageScenario usageScenario) {
		...
		final IUserFactory userFactory = new ClosedWorkloadUserFactory(this.rootContext.getModel(), closedWorkload.getThinkTime_ClosedWorkload().getSpecification(), usageScenario) {
			@Override
	...
	private de.uka.ipd.sdq.simucomframework.usage.OpenWorkload getOpenWorkloadDriver(final Workload workload, final UsageScenario usageScenario) {
		if (LOGGER.isDebugEnabled()) {
		...
		// create workload driver by using given factory
		return new de.uka.ipd.sdq.simucomframework.usage.OpenWorkload(this.rootContext.getModel(), userFactory, openWorkload.getInterArrivalTime_OpenWorkload().getSpecification());
	}
	...
	private IScenarioRunner getScenarioRunner(final UsageScenario scenario) {
		return new IScenarioRunner() {
			...
			public void scenarioRunner(final SimuComSimProcess thread) {
				...
				final UsageScenario myUsageScenarioCopy = (UsageScenario) usageModel.eResource().getEObject(scenario.getId());
