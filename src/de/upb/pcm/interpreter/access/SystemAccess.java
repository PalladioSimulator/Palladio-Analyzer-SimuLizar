...
public class SystemAccess extends AbstractPCMModelAccess<System> {
	 ...
	 */
	public AssemblyConnector getConnectedAssemblyConnector(
			final RequiredRole requiredRole) {
		for (final Connector connector : getModel()
				.getConnectors__ComposedStructure()) {
			if (connector.eClass() == CompositionPackage.eINSTANCE
					.getAssemblyConnector()) {
				if (((AssemblyConnector) connector)
						.getRequiredRole_AssemblyConnector().equals(
								requiredRole)) {
	 ...
	 */
	public ProvidedDelegationConnector getConnectedProvidedDelegationConnector(
