...
public class PCMStartInterpretationJob implements
	...
	public void execute(final IProgressMonitor monitor)
		...
		final SimuLizarRuntimeState runtimeState = new SimuLizarRuntimeState(
				configuration, new ModelAccess(this.blackboard));
		IExtension[] extensions = Platform
				.getExtensionRegistry()
				.getExtensionPoint(
						"org.palladiosimulator.simulizar.runtimestate.runtimestateaccessor")
				.getExtensions();
		for (IExtension extension : extensions) {
			for (IConfigurationElement element : extension
				...
				try {
					accessor = (IRuntimeStateAccessor) element
							.createExecutableExtension("accessor");
