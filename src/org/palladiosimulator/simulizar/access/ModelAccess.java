...
/**
 * Helper to access the PCM model (global and local), the RuntimeMeasurement
 * model, the Monitor Repository model, the usage evolution model and all SD
 * models.
 * 
...
public class ModelAccess implements IModelAccess, IReconfigurationListener {
	...
	private static final Logger LOGGER = Logger.getLogger(ModelAccess.class
			.getName());
	...
	public ModelAccess(final MDSDBlackboard blackboard) {
		...
		this.runtimeMeasurementModel = RuntimeMeasurementFactory.eINSTANCE
				.createRuntimeMeasurementModel();
		this.pcmPartition = getResourceSetPartition(blackboard,
	...
	private PCMResourceSetPartition copyPCMPartition() {
		...
		List<EObject> modelCopy = EMFCopyHelper
				.deepCopyToEObjectList(pcmPartition.getResourceSet());
		for (int i = 0; i < modelCopy.size(); i++) {
			Resource resource = newPartition.getResourceSet().createResource(
					URI.createFileURI("/temp" + i));
	...
	public MonitorRepository getMonitorRepositoryModel() {
		try {
			...
			List<MonitorRepository> result = this.pcmPartition
					.getElement(MonitorRepositoryPackage.eINSTANCE
	...
	public ServiceLevelObjectiveRepository getServiceLevelObjectiveRepositoryModel() {
		try {
			...
			List<ServiceLevelObjectiveRepository> result = this.pcmPartition
					.getElement(ServicelevelObjectivePackage.eINSTANCE
	...
	/**
	 * 
	 * @return the global usage evolution model, or null if no such model is
	 *         available
	 */
	@Override
	public UsageEvolution getUsageEvolutionModel() {
		try {
			...
			List<UsageEvolution> result = this.pcmPartition
					.getElement(UsageevolutionPackage.eINSTANCE
							.getUsageEvolution());
			return result.get(0);
		} catch (Exception e) {
	...
	private <T extends ResourceSetPartition> T getResourceSetPartition(
			final MDSDBlackboard blackboard, final String id) {
