...
public class ModelRepository {
	...
	public void initialize(ResourceSet resourceSet,
			eurema.ModelResourceSet eModelResourceSet) {
		logger.debug("Initializing the EUREMA model repository...");
		if (resourceSet == null) {
			throw new EuremaInterpreterException(
					"Failure in initializing the EUREMA model repository. "
							+ "The EMF resource set must not be null.");
		}
	...
	public Model loadModel(String uri) {
		...
		eurema.ModelResource eModelResource = this
				.retrieveModelResourceByURI(modelURI.toString());
	...
	public Model loadModelOnDemand(String uri, String name) {
		if (uri == null || uri.equals("") || name == null || name.equals("")) {
			throw new ModelRepositoryException(
					"Paramter \"uri\" or \"name\" to load a model "
		...
		eurema.ModelResource modelResource = this
				.retrieveModelResourceByURI(modelURI.toString());
	...
	public Model createModel(String uri, String name) {
		if (uri == null || uri.equals("") || name == null || name.equals("")) {
			throw new ModelRepositoryException(
					"Paramter \"uri\" or \"name\" to create a "
							+ "model must not be null or the empty String.");
		}
	...
	private eurema.ModelResource createModelResource(String uri, String name) {
		if (uri == null || uri.equals("") || name == null || name.equals("")) {
			throw new ModelRepositoryException(
					"Paramters \"uri\" or \"name\" to create a "
							+ "model resource must not be null or the empty String.");
		}
		eurema.ModelResource modelResource = Metamodel.INSTANCE
				.getEuremaFactory().createModelResource();
	...
	private eurema.ModelResource retrieveModelResourceByURI(String uri) {
		if (uri == null || uri.equals("")) {
			throw new ModelRepositoryException(
					"Paramters \"uri\" to retrieve a model "
							+ "resource must not be null or the empty String.");
		}
		eurema.ModelResource modelResource = null;
		for (eurema.ModelResource r : this.eModelResourceSet.getResources()) {
			String rUri = (String) r.getURI();
			if (uri.equals(rUri)) {
				modelResource = r;
				break;
			}
	...
	public void addURIMapping(String source, String target) {
		this.resourceSet.getURIConverter().getURIMap()
				.put(URI.createURI(source), URI.createURI(target));
	}
	...
	public EPackage getPackage(String nsURI) {
		if (nsURI == null || nsURI.equals("")) {
			throw new ModelRepositoryException(
					"Paramter \"nsUri\" to load an EPackage must "
	...
	public Object registerPackage(EPackage ePackage) {
		return this.resourceSet.getPackageRegistry().put(ePackage.getNsURI(),
				ePackage);
	}
	...
	public void saveResourceToDisk(Resource resource) {
		if (resource == null) {
			throw new ModelRepositoryException(
					"Paramter \"resource\" to save a resource must not be null.");
		}
	...
	public void saveResourceToDisk(Model modelResource) {
		if (modelResource == null) {
			throw new ModelRepositoryException(
					"Paramter \"modelResource\" to save a resource must not be null.");
		}
	...
	public boolean containsEMFResource(URI uri) {
		if (uri == null) {
			throw new ModelRepositoryException(
					"Paramter \"uri\" to check containment of "
	...
	public boolean containsModel(URI uri) {
		if (uri == null) {
			throw new ModelRepositoryException(
					"Paramter \"uri\" to check containment of "
							+ "a model in the repository must not be null.");
		}
		...
		if (emfContained & modelContained) {
			...
			throw new ModelRepositoryException(
					"Inconsistent model repository: URI " + uri.toString()
							+ " is used\n" + " * in the EURAMA model? "
							+ modelContained + "\n"
	...
	public Model addModel(Resource resource, String name) {
		if (resource == null || name == null || name.equals("")) {
			throw new ModelRepositoryException(
					"Paramter \"resource\" must not be null or \"name\" must "
							+ "not be null or the empty String in order to "
							+ "add a model to the repository.");
		}
		URI resourceURI = resource.getURI();
		logger.debug("Add the following resource to the repository: "
				+ resourceURI.toString());
		if (this.containsModel(resourceURI)) {
			eurema.ModelResource eModelResource = this
					.retrieveModelResourceByURI(resourceURI.toString());
			return new Model(eModelResource, resource);
		} else {
			eurema.ModelResource modelResource = this.createModelResource(
					resourceURI.toString(), name);
			this.eModelResourceSet.getResources().add(modelResource);
	...
	public String listResources() {
		...
		int j = 1;
		for (eurema.ModelResource modelResource : this.eModelResourceSet
				.getResources()) {
