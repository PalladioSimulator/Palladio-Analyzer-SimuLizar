...
public class ExecutionManagerImpl implements ExecutionManager,
	...
	private static Logger logger = LogManager
			.getLogger(ExecutionManagerImpl.class);
	...
	public EventQueue initialize(eurema.RuntimeEnvironment eRuntimeEnvironment) {
		...
		if (this.initialized) {
			throw new EuremaInterpreterException(
					"Execution manager has already been initialized.");
		}
		this.globalSensorEventQueue = new EventQueueImpl(
				"EUREMA_Global Sensor Event Queue");
	 ...
	 */
	private void setUpModuleExecutables(
			eurema.RuntimeEnvironment eRuntimeEnvironment) {
		logger.debug("Setting up the executables for each managing module...");
		this.modulesToExecutables = new HashMap<>();
		List<eurema.Module> allManagingModules = ArchitectureManager.INSTANCE
				.getAllManagingModules();
		for (eurema.Module eModule : allManagingModules) {
			...
			if (eModule instanceof eurema.MegamodelModule) {
				moduleExecutable = new MegamodelModuleExecutableImpl(
						(eurema.MegamodelModule) eModule, this);
				// Connect eurema.ExecutionContext to the
				// eurema.RuntimeEnvironment
				moduleExecutable.getExecutionContext().setEnvironment(
						eRuntimeEnvironment);
			} else if (eModule instanceof eurema.SoftwareModule) {
				moduleExecutable = new SoftwareModuleExecutableImpl(
						(eurema.SoftwareModule) eModule, this);
			}
	 ...
	 */
	private void setUpModuleSchedulers(
		...
		List<eurema.Module> adaptableSoftwareModules = ArchitectureManager.INSTANCE
				.getAllManagedModules();
				...
				if (scheduler != null) {
					...
					if (scheduler instanceof EventDrivenModuleScheduler) {
						...
						this.addEventSubscription(
								((EventDrivenModuleScheduler) scheduler),
								eventSubscriptions);
					}
				}
			}
	...
	private ModuleScheduler createScheduler(eurema.Sensing eSensing) {
		...
		if (eSensing.getTrigger() != null) {
			...
			ModuleExecutable<?, ?, ?, ?> moduleExecutable = this
					.getExecutable(eModule);

			// check which type of trigger is used
			if (eTrigger.getEvents() == null
					|| eTrigger.getEvents().size() == 0) {
				...
				if (eTrigger.getPeriod() == 0) {
					...
					String msg = "Failure in setting up the execution manager. The module "
							+ eModule.getName()
							+ " has an invalid triggering condition."
							+ " It specified neither an event nor a period.";
					logger.debug(" |_ " + msg);
					throw new EuremaInterpreterException(msg);
				} else {
					// It is a PERIODIC triggering condition
					scheduler = new PeriodicModuleSchedulerImpl(
							moduleExecutable);
					logger.debug(" |_ Creating the scheduler for the periodic "
							+ eModule.eClass().getName() + " "
							+ eModule.getName());
				}
			} else {
				...
				if (eTrigger.getPeriod() == 0) {
					...
					scheduler = new EventDrivenModuleSchedulerImpl(
							moduleExecutable);
					logger.debug(" |_ Creating the scheduler for the event-driven "
							+ eModule.eClass().getName()
							+ " "
							+ eModule.getName());
				} else {
					...
					scheduler = new EventPeriodicModuleSchedulerImpl(
							moduleExecutable);
					logger.debug(" |_ Creating the scheduler for the event-periodic "
							+ eModule.eClass().getName()
							+ " "
							+ eModule.getName());
				}
			...
			logger.debug("No triggering condition has been specified for the module "
					+ eSensing.getSource().getName());
			return null;
		}
	}
	...
	private void addEventSubscription(
		...
		for (eurema.Event eEvent : eEvents) {
			...
			List<EventDrivenModuleScheduler> subscribers = eventSubscriptions
					.get(eEventType);
			if (subscribers == null) {
			...
			logger.debug("Added subscription of the scheduler for module "
					+ scheduler.getExecutable().getEModule().getName()
	...
	private void startModuleSchedulers() {
		...
		for (ModuleScheduler scheduler : this.schedulers) {
			logger.debug(" |_ Starting the scheduler for module "
					+ scheduler.getExecutable().getEModule().getName() + " ...");
	 ...
	 *            interested in events of these types
	 */
	private void setUpDemultiplexer(
			Map<eurema.EventType, List<EventDrivenModuleScheduler>> eventSubscriptions) {
		logger.debug("Setting up the demultiplexer ...");
		this.demultiplexer = new DemultiplexerImpl(this.globalSensorEventQueue,
				eventSubscriptions);
	}
	...
	public ModuleExecutable<?, ?, ?, ?> getExecutable(eurema.Module eModule) {
		ModuleExecutable<?, ?, ?, ?> result = this.modulesToExecutables
				.get(eModule);
		assert result != null;
		return result;
	}
