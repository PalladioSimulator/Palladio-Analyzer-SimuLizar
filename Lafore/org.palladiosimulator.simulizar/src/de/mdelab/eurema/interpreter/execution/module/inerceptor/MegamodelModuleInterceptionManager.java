...
public class MegamodelModuleInterceptionManager {
	 ...
	 */
	public final static MegamodelModuleInterceptionManager INSTANCE = new MegamodelModuleInterceptionManager();

	/**
	 * Logging.
	 */
	private static Logger logger = LogManager.getLogger(MegamodelModuleInterceptionManager.class);
	 ...
	 */
	public void executeInterceptorModules(eurema.MegamodelModule interceptedModule, eurema.Event interceptionEvent) {
		// check if the inteceptingModule is sensed by another module
		EList<eurema.Sensing> eSensingRelationships = interceptedModule.getSensedBy();
		for (eurema.Sensing eSensing : eSensingRelationships) {
			...
			for (eurema.Event eTriggerEvent : eTrigger.getEvents()) {
				if (eTriggerEvent.getName().equals(interceptionEvent.getName())
						& eTriggerEvent.getType().getType().equals(interceptionEvent.getType().getType())) {
			...
			if (matched) {
				...
				ModuleExecutable<?, ?, ?, ?> interceptingModuleExecutable = this.globalExecutionContext
				...
				if (interceptingModule instanceof eurema.MegamodelModule) {
					...
					eurema.InitialOperation eInitialOperation = ((eurema.MegamodelModuleTrigger) eTrigger)
							.getInitialOperation();

					// run the executable
					logger.debug("Intecepting the megamodel module " + interceptedModule.getName() + " with the event "
							+ interceptionEvent.getType().getType() + "[" + interceptionEvent.getName()
							+ "] to execute the megamodel module " + interceptingModule.getName()
							+ " with the initial operation " + eInitialOperation.getName() + " ...");

					@SuppressWarnings("unchecked")
					ModuleExecutable<eurema.MegamodelModule, eurema.MegamodelModuleTrigger, eurema.InitialOperation, eurema.FinalOperation> megamodelModuleExecutable = (ModuleExecutable<eurema.MegamodelModule, eurema.MegamodelModuleTrigger, eurema.InitialOperation, eurema.FinalOperation>) interceptingModuleExecutable;
					eurema.FinalOperation result = megamodelModuleExecutable.run(eInitialOperation);
					logger.debug("Megamodel module " + interceptingModule.getName()
							+ " has been executed and terminated in state " + result.getName() + ".");
