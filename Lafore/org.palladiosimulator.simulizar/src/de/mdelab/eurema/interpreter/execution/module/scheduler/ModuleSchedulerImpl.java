...
public abstract class ModuleSchedulerImpl implements ModuleScheduler {
	 ...
	 * Logging.
	 */
	private static Logger logger = LogManager.getLogger(ModuleSchedulerImpl.class);
	 ...
	 */
	private final ExecutionPermissionManager executionPermissionManager;
	...
	public ModuleSchedulerImpl(ModuleExecutable<?, ?, ?, ?> moduleExecutable) {
		...
		this.executionPermissionManager = ConsistencyManagementFactory.INSTANCE.getExecutionPermissionManager();
	}
	...
	protected boolean matchTriggeringCondition(eurema.Event eSensorEvent) {
		...
		if (matched) {
			logger.debug("Matching sensor event " + EuremaUtils.eventToString(eSensorEvent)
	 ...
	 *         <code>false</code>.
	 */
	private boolean matchEvents(eurema.Event eSensorEvent, EList<eurema.Event> eConditionEvents) {
		...
		for (eurema.Event eConditionEvent : eConditionEvents) {
			...
			if (!typeMatched) {
				typeMatched = this.matchEventTypes(eSensorEventType, eConditionEventType.getSubTypes());
			}
			// types are matched; match event names
			if (typeMatched) {
				String eConditionEventName = eConditionEvent.getName();
				if (eConditionEventName != null && !(eConditionEventName.equals(""))) {
	 ...
	 */
	private boolean matchEventTypes(eurema.EventType eSensorEventType, EList<eurema.EventType> eConditionEventTypes) {
		for (eurema.EventType eConditionEventType : eConditionEventTypes) {
			if (eSensorEventType.equals(eConditionEventType)) {
				...
				// condition.
				this.matchEventTypes(eSensorEventType, eConditionEventType.getSubTypes());
			}
		}
		return false;
	}
	...
	protected void runExecutable() {
		...
		this.executionPermissionManager.waitForExecutionPermission(this.moduleExecutable);
		...
		if (eModule instanceof eurema.MegamodelModule) {
			...
			eurema.InitialOperation eInitialOperation = eMegamodelTrigger.getInitialOperation();
			...
			eurema.FinalOperation result = megamodelModuleExecutable.run(eInitialOperation);
			logger.debug("Megamodel module " + eModule.getName() + " has been executed and terminated in state "
					+ result.getName() + ".");
		...
		this.executionPermissionManager.releaseExecutionPermission(this.moduleExecutable);
	}
	...
	protected eurema.Event createTimerEvent(String eventName) {
		eurema.Event initialTimerEvent = Metamodel.INSTANCE.getEuremaFactory().createEvent();
		initialTimerEvent.setType(EuremaEventTypeHierarchy.INSTANCE.getEEventType(EuremaEventType.TIMER));
	...
	protected EventQueueInternal createLocalSensorEventQueue() {
		String queueName = "EUREMA_SensorEventQueue_" + moduleExecutable.getEModule().getName();
	...
	protected EventQueueInternal createLocalTimerEventQueueName() {
		String queueName = "EUREMA_TimerEventQueue_" + this.moduleExecutable.getEModule().getName();
		return new EventQueueImpl(queueName);
	}
}
