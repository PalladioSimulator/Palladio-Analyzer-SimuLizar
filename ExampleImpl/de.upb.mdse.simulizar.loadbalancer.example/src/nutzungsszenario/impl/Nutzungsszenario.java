...
public class Nutzungsszenario implements java.lang.Runnable {
	...
	public Nutzungsszenario() {

		{
			...
			// Wait for system
			m_portAngeboten_ILastverteiler = (defaultrepository.ILastverteiler) de.uka.ipd.sdq.prototype.framework.registry.RmiRegistry
					.lookup("ILastverteiler_EntpackerService");
		}

		// FIXME: why is expRun needed?
		// expRun =
		// de.uka.ipd.sdq.prototype.framework.AbstractMain.getLatestExperimentRun();
		...
		// .initialiseStoExCache(new
		// de.uka.ipd.sdq.probfunction.math.impl.DefaultRandomGenerator());

		de.uka.ipd.sdq.probfunction.math.IProbabilityFunctionFactory probFunctionFactory = de.uka.ipd.sdq.probfunction.math.impl.ProbabilityFunctionFactoryImpl
				.getInstance();

		probFunctionFactory
				.setRandomGenerator(new de.uka.ipd.sdq.probfunction.math.impl.DefaultRandomGenerator());
		de.uka.ipd.sdq.simucomframework.variables.cache.StoExCache
				.initialiseStoExCache(probFunctionFactory);
	...
	public void scenarioRunner() {
		de.uka.ipd.sdq.simucomframework.variables.stackframe.SimulatedStackframe<Object> outerStackframe = ctx
...

		{
			...
			try {
				...
				stackframe.addValue("datei.BYTESIZE", ctx.evaluate(
						"IntPMF[ (100;0.5) (50;0.5) ]", currentFrame));

				de.uka.ipd.sdq.simucomframework.variables.stackframe.SimulatedStackframe<Object> callResult =

				m_portAngeboten_ILastverteiler.entpacke0(ctx);
