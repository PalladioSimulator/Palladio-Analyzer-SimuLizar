 ...
 * 
 */
public class QuiescenceManagerImpl implements QuiescenceManager, ExecutionPermissionManager {

	/**
	 * Logging.
	 */
	private static Logger logger = LogManager.getLogger(QuiescenceManagerImpl.class);
	 ...
	 */
	private final Map<ModuleExecutable<?, ?, ?, ?>, Thread> executingModules;

	/**
	 * List of listeners to be notified about changes in the quiescent state.
	 */
	private final List<QuiescenceListener> listeners;
	 ...
	 */
	@Override
	public void waitForExecutionPermission(ModuleExecutable<?, ?, ?, ?> moduleExecutable) {
	 ...
	 */
	@Override
	public void releaseExecutionPermission(ModuleExecutable<?, ?, ?, ?> moduleExecutable) {
		Thread currentSchedulerThread = Thread.currentThread();
		synchronized (this) {
			...
			Thread executingThread = this.executingModules.get(moduleExecutable);
	...
	public void activateQuiescence() {
		...
		synchronized (this) {
			if (this.getCurrentState() == QuiescenceState.OFF) {
				...
				throw new QuiescenceManagmentException("Cannot switch Quiescence state from " + this.getCurrentState()
						+ " to " + QuiescenceState.BLOCKING);
			}
		}
	}
		...
		synchronized (this) {
			if (this.getCurrentState() != QuiescenceState.OFF) {
				...
				throw new QuiescenceManagmentException(
						"Cannot switch Quiescence state from " + this.getCurrentState() + " to " + QuiescenceState.OFF);
			}
		}
	}
	...
	private void setCurrentState(QuiescenceState newState) {
		...
		if (newState == oldState) {
			throw new QuiescenceManagmentException("Cannot set quiescence state from " + oldState + " to " + newState
					+ " because there is no change in the state.");
		}
		this.quiescenceState = newState;
		logger.debug("Quiescence state has changed from " + oldState + " to " + newState);
		// notify listeners
