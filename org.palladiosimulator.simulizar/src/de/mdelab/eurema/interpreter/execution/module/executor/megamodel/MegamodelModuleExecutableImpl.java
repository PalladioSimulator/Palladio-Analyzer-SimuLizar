 ...
 * 
 */
public class MegamodelModuleExecutableImpl implements
	 ...
	 * Logging.
	 */
	private static Logger logger = LogManager.getLogger(MegamodelModuleExecutableImpl.class);

	/**
	 * The global execution context.
	 */
	private final GlobalExecutionContext globalExecutionContext;

	/**
	 * The megamodel module to be executed.
	 */
	private final eurema.MegamodelModule eMegamodelModule;
	 ...
	 *            the global execution context
	 */
	public MegamodelModuleExecutableImpl(eurema.MegamodelModule eMegamodelModule,
		...
		eurema.ExecutionContext eExecutionContext = this.setUpExecutionContext();
	...
	public eurema.FinalOperation run(eurema.InitialOperation eInitialOperation) {
		...
		synchronized (this) {
			...
			logger.debug("Starting to run the megamodel module " + this.eMegamodelModule.getName() + "... by thread "
					+ this.executingThread.getId());

			if (eInitialOperation == null) {
				throw new EuremaInterpreterException("Failure in executing the megamodel module "
						+ this.eMegamodelModule.getName() + ". The initial operation to start execution is null.");
			}
			...
			// operation
			eurema.EuremaPackage euremaPackage = Metamodel.INSTANCE.getEuremaPackage();
			...
			while (!executableEClass.equals(finalOperationEClass)) {
				...
				eurema.Executable nextExecutable = executor.execute(executable, executedBefore, this);
			...
			logger.debug("Terminating the execution of the megamodel module " + this.eMegamodelModule.getName()
					+ "... by thread " + this.executingThread.getId());
	 ...
	 */
	private void checkForInteceptions(EuremaEventType eventType, eurema.Executable executable) {
		if (executable instanceof eurema.OperationBehavior) {
			eurema.Event eEvent = Metamodel.INSTANCE.getEuremaFactory().createEvent();
			eEvent.setName(executable.getName());
			eEvent.setType(EuremaEventTypeHierarchy.INSTANCE.getEEventType(eventType));
			MegamodelModuleInterceptionManager.INSTANCE.executeInterceptorModules(this.eMegamodelModule, eEvent);
		}
	...
	private eurema.ExecutionContext setUpExecutionContext() {
		logger.debug("Setting up the execution context of the megamodel module " + this.eMegamodelModule.getName());
		...
		eurema.ExecutionContext eExecutionContext = Metamodel.INSTANCE.getEuremaFactory().createExecutionContext();
		String executionContextId = this.eMegamodelModule.getUid() + "_EXECUTION_CONTEXT";
	 ...
	 */
	private void setUpExecutionInformation(eurema.ExecutionContext eExecutionContext) {
		logger.debug("Setting up the execution information of the megamodel module " + this.eMegamodelModule.getName());
		...
		while (allContents.hasNext()) {
			...
			if (next instanceof eurema.Executable) {
				...
				// element
				eurema.ExecutionInformation eExecutionInformation = eFactory.createExecutionInformation();
	...
	public eurema.ExecutionContext getExecutionContext() {
		eurema.ExecutionContext eExecutionContext = this.eMegamodelModule.getContext();
	 ...
	 * 
	 * @param name
	 *            the name of the {@code eurema.Operation} or
	 *            {@code eurema.Exit}.
	...
	public long retrieveCount(String query) {
		eurema.ExecutionInformation eExecutionInformation = this.resolveExecutionInformation(query);
	...
	public long retrieveTime(String query) {
		eurema.ExecutionInformation eExecutionInformation = this.resolveExecutionInformation(query);
		long time = eExecutionInformation.getTime();
	...
	private eurema.ExecutionInformation resolveExecutionInformation(String query) {
		if (query == null || query.equals("")) {
			throw new EuremaInterpreterException("Failure in resolving an executable megamodel element and its "
		...
		if (!query.contains(QueryExecutionInformation.SEPARATOR)) {
			...
			if (executable == null) {
				throw new EuremaInterpreterException("Failure in resolving an executable megamodel element and its "
						+ " execution information. No element can be found for the query: " + query);
			}
			return executable.getExecInfo();
		} else {
			// The query is the qualified name of a transition
			String[] queryParts = query.split(QueryExecutionInformation.SEPARATOR);
			assert queryParts.length == 2;
			String queryOperationName = queryParts[0];
			String queryExitName = queryParts[1];

			eurema.Operation eOperation = this.getOperationByName(queryOperationName);
			if (eOperation == null) {
				throw new EuremaInterpreterException("Failure in resolving an operation and its execution information. "
						+ "No operation can be found for the query: " + queryOperationName);
			}
			...
			if (eResult == null) {
				throw new EuremaInterpreterException("Failure in resolving a transition and its execution information. "
						+ "No transition can be found for the query: " + query);
			}
	...
	private void setCurrentPointer(eurema.Executable executable) {
		if (executable != null) {
			logger.debug(" |_ Executing the " + executable.eClass().getName() + " " + executable.getName() + " ...");
		} else {
	 ...
	 */
	private void updateExecutionInformation(eurema.Executable executable, eurema.Executable executedBefore) {
		eurema.ExecutionInformation executableExecutionInfo = executable.getExecInfo();
		...
		if (executable instanceof eurema.Operation) {
			...
			// <tt>count</tt> by 1
			executableExecutionInfo.setCount(executableExecutionInfo.getCount() + 1);
			...
			for (eurema.Exit eExit : sourceOperation.getExits()) {
				...
				if (!eTransition.equals(executable)) {
					eurema.ExecutionInformation eTransitionExecutionInfo = eTransition.getExecInfo();
					eTransitionExecutionInfo.setCount(eTransitionExecutionInfo.getCount() + 1);
				}
			}
