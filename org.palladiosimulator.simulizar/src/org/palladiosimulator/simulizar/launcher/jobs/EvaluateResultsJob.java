...
public class EvaluateResultsJob extends SequentialBlackboardInteractingJob<MDSDBlackboard> {
	...
	public void execute(IProgressMonitor arg0) throws JobFailedException, UserCanceledException {
		...
		PCMResourceSetPartition partition = (PCMResourceSetPartition) this.getBlackboard()
				.getPartition(LoadPCMModelsIntoBlackboardJob.PCM_MODELS_PARTITION_ID);

		if (partition == null) {
			LOGGER.info("No Service level objectives provided. Skipping evaluation of experiment data");
		} else {
			String repositoryId = (String) this.configuration.getAttributes()
					.get(EDP2RecorderConfigurationFactory.REPOSITORY_ID);
	...
	/**
	 * Finds the measurements referenced by the SLO in the given measurements
	 * lists. For identification, this methods tries to match metric IDs and
	...
	private Measurement findMeasurement(final List<Measurement> measurementList,
		...
		for (final Measurement measurement : measurementList) {
			if (containsMetric(measurement.getMeasuringType().getMetric(),
				...
				// TODO Comparing the name of Measuring points is not the best
				// solution (as the name
				// is generally not unique). I see three options, all requiring
				// some architectural
				// refactoring at several places: (1) Require that every
				// measuring point is an URI
				// measuring point -- then, the URI could be used to uniquely
				// identify the measuring
				// point; (2) just use 1 measuring point for the same entity at
				// a time -- then two
				// equal measuring points objects literally point to the same
				// entity. For the
				// latter, we should attach measuring points directly to PCM
				// elements and make them
				// intrinsic part of such elements; (3) create a
				// MeasuringPointUtility method for
				// generating a measuring points ID based on the type of
				// measuring point; URI
				// measuring points should generate the same ID as dedicated PCM
				// measuring points if
				// they reference the same entity. Options should be discussed
				// if this becomes an
