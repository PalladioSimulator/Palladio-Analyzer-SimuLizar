...
/**
 * QVTo executor helper class that supports executing QVTo reconfiguration
 * rules.
 * 
...
public class QVTOExecutor {
	 ...
	 */
	public QVTOExecutor(final IModelAccess modelAccess,
	...
	/**
	 * Executes all QVTo rules found in the configured reconfiguration rule
	 * folder.
	 * 
	...
	private File getFolder(String path) {
		...
		if (!path.startsWith("platform:")) {
			...
			try {
				...
				LOGGER.warn(
						"No QVTo rules found, QVTo reconfigurations disabled.",
						e);
			}
		}
		return folder;
	...
	private boolean execute(final TransformationExecutor executor) {
		...
		List<PCMModelElementMeasurement> runtimeModel = this.modelAccess
				.getPRMModel().getPcmModelElementMeasurements();
		List<EObject> pcmAllocation = Arrays.asList((EObject) this.modelAccess
				.getGlobalPCMModel().getAllocation());
		List<EObject> pcmSystem = Arrays.asList((EObject) this.modelAccess
				.getGlobalPCMModel().getSystem());
		List<EObject> pcmResources = Arrays.asList((EObject) this.modelAccess
				.getGlobalPCMModel().getResourceTypeRepository());
		List<EObject> pcmRepository = new ArrayList<EObject>();
		pcmRepository.addAll(this.modelAccess.getGlobalPCMModel()
				.getRepositories());
		...
		ExecutionDiagnostic result = executor.execute(exContext, input,
				inoutRepository, inoutSystem, inoutAllocation, inoutResources);

		// check the result for success
		if (result.getSeverity() == Diagnostic.OK) {
			LOGGER.log(
					Level.INFO,
					"Rule application successfull with message: "
							+ result.getMessage());
			return true;
		} else {
			LOGGER.log(Level.WARN, "Rule application failed with message: "
					+ result.getMessage());
			return false;
		}
	}
