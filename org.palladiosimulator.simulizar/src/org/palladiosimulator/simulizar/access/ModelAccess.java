...
import org.palladiosimulator.simulizar.launcher.jobs.LoadDEModelIntoBlackBoardJob;
...
public class ModelAccess implements IModelAccess, IReconfigurationListener {
	...
	private final DEMResourceSetPartition demPartition;
	...
	public ModelAccess(final MDSDBlackboard blackboard) {
		...
		this.demPartition = getResourceSetPartition(blackboard,
				LoadDEModelIntoBlackBoardJob.DE_MODEL_PARTITION_ID);
		this.sdmPartition = getResourceSetPartition(blackboard,
				LoadSDMModelsIntoBlackboardJob.SDM_MODEL_PARTITION_ID);
		this.pmsPartition = getResourceSetPartition(blackboard,
	...
	private ModelAccess(final ModelAccess copy) {
		...
		this.demPartition = copy.demPartition;
	...
	/**
	 * Checks whether sdm models exists, without using any classes from sd
	 * interpreter.
	 * 
	...
	public void reconfigurationExecuted(
			final Collection<Notification> modelChanges) {
