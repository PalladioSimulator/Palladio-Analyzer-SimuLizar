...
public class UpdateChangeListener extends EContentAdapter {
	...
	private static Logger logger = LogManager
			.getLogger(UpdateChangeListener.class);
	...
	private void createAndAddLayerChangeEvent(eurema.Architecture notifier,
		...
		if (featureID == eurema.EuremaPackage.ARCHITECTURE__LAYERS) {
			...
			if (eventType == Notification.REMOVE) {
				...
				LayerRemovedChangeEvent event = new LayerRemovedChangeEvent(
						removedLayer);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
				logger.debug("Layer #" + removedLayer.getNumber()
						+ " called \"" + removedLayer.getName()
				...
				LayerAddedChangeEvent event = new LayerAddedChangeEvent(
						addedLayer);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
				logger.debug("Layer #" + addedLayer.getNumber() + " called \""
						+ addedLayer.getName()
				...
				for (eurema.Layer addedLayer : addedLayers) {
					LayerAddedChangeEvent event = new LayerAddedChangeEvent(
							addedLayer);
					UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
					logger.debug("Layer #" + addedLayer.getNumber()
							+ " called \"" + addedLayer.getName()
							+ "\" has been added to the architecture.");
				}
			}
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void createAndAddModuleChangeEvent(eurema.Layer notifier,
			Notification notification) {
		...
		if (featureID == eurema.EuremaPackage.LAYER__MODULES) {
			...
			if (eventType == Notification.REMOVE) {
				...
				ModuleRemovedChangeEvent event = new ModuleRemovedChangeEvent(
						removedModule, notifier);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
				...
				ModuleAddedChangeEvent event = new ModuleAddedChangeEvent(
						addedModule, notifier);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
	 ...
	 *            layer.
	 */
	private void logModuleChange(eurema.Layer notifier, eurema.Module module,
			boolean isRemoved) {
		String verb = "";
		if (isRemoved) {
			verb = "removed from";
		...
		if (module instanceof eurema.MegamodelModule) {
			logger.debug("MegamodelModule \""
					+ EuremaUtils.getFqModuleName(module) + "\" (ID = "
					+ module.getUid() + ") has been " + verb + " the layer #"
					+ notifier.getNumber() + " \"" + notifier.getName() + "\".");
		} else if (module instanceof eurema.SoftwareModule) {
			logger.debug("SoftwareModule \""
					+ EuremaUtils.getFqModuleName(module) + "\" (ID = "
					+ module.getUid() + ") has been " + verb + " the layer #"
					+ notifier.getNumber() + " \"" + notifier.getName() + "\".");
		}
	 ...
	 */
	private void createAndAddRebindMegamodelCallChangeEvent(
			eurema.MegamodelCall notifier, Notification notification) {
		...
		if (featureID == eurema.EuremaPackage.MEGAMODEL_CALL__BINDING) {
			...
			if (eventType == Notification.SET) {
				...
				RebindMegamodelCallChangeEvent event = new RebindMegamodelCallChangeEvent(
						notifier, oldModule, newModule);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
				logger.debug("The MegamodelCall \""
						+ notifier.getName()
						+ "\" of the MegamodelModule \""
						+ EuremaUtils.getFqModuleName(notifier.getMegamodel()
								.getModule())
						+ "\" has been rebound from the MegamodelModule \""
						+ EuremaUtils.getFqModuleName(oldModule)
						+ "\" to the MegamodelModule \""
						+ EuremaUtils.getFqModuleName(newModule) + "\".");
			}
		}
	}
	 ...
	 *            the event notifying about the rebinding.
	 */
	private void createAndAddRebindModelOperationChangeEvent(
			eurema.ModelOperation notifier, Notification notification) {
		...
		if (featureID == eurema.EuremaPackage.MODEL_OPERATION__BINDING) {
			...
			if (eventType == Notification.SET) {
				...
				RebindModelOperationChangeEvent event = new RebindModelOperationChangeEvent(
						notifier, oldModule, newModule);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
				logger.debug("The ModelOperation \""
						+ notifier.getName()
						+ "\" of the MegamodelModule \""
						+ EuremaUtils.getFqModuleName(notifier.getMegamodel()
								.getModule())
						+ "\" has been rebound from the SoftwareModule \""
						+ EuremaUtils.getFqModuleName(oldModule)
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void createAndAddTriggerChangeEvent(eurema.Trigger notifier,
			Notification notification) {
		...
		if (featureID == eurema.EuremaPackage.TRIGGER__PERIOD) {
			...
			if (eventType == Notification.SET) {
				...
				TriggerPeriodChangeEvent event = new TriggerPeriodChangeEvent(
						notifier, oldPeriod, newPeriod);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
				...
				if (notifier instanceof eurema.MegamodelModuleTrigger) {
					...
					logger.debug("The trigger of the MegamodelModule \""
							+ EuremaUtils.getFqModuleName(mmTrigger.getModule())
							+ "\" has changed its period from " + oldPeriod
							+ " to " + newPeriod + ".");
					...
					logger.debug("The trigger of the SoftwareModule \""
							+ EuremaUtils.getFqModuleName(swTrigger.getModule())
							+ "\" has changed its period from " + oldPeriod
							+ " to " + newPeriod + ".");
				}
			}
			...
			if (eventType == Notification.REMOVE) {
				...
				event = new TriggerEventChangeEvent(notifier,
						Collections.<eurema.Event> emptyList(), removedEvents);
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
			} else if (eventType == Notification.REMOVE_MANY) {
				Object oldValues = notification.getOldValue();
				...
				event = new TriggerEventChangeEvent(notifier, addedEvents,
						Collections.<eurema.Event> emptyList());
				UpdateChangeEventQueue.INSTANCE.getQueue().add(event);
			} else if (eventType == Notification.ADD_MANY) {
				Object newValues = notification.getNewValue();
			...
			if (event != null) {
				...
				if (trigger instanceof eurema.MegamodelModuleTrigger) {
					eurema.MegamodelModule mmModule = ((eurema.MegamodelModuleTrigger) trigger)
							.getModule();
					sb.append("MegamodelModule \""
							+ EuremaUtils.getFqModuleName(mmModule) + "\" ");
				} else if (trigger instanceof eurema.SoftwareModuleTrigger) {
					eurema.SoftwareModule swModule = ((eurema.SoftwareModuleTrigger) trigger)
							.getModule();
					sb.append("SoftwareModule \""
							+ EuremaUtils.getFqModuleName(swModule) + "\" ");
				}
