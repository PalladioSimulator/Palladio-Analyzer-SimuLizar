 ...
 * 
 */
public class EventPeriodicModuleSchedulerImpl extends ModuleSchedulerImpl
		implements EventPeriodicModuleScheduler {

	/**
	 * Logging.
	 */
	private static Logger logger = LogManager
			.getLogger(EventPeriodicModuleSchedulerImpl.class);

	/**
	 * The local queue from which the scheduler consumes sensor events.
	 */
	private EventQueueInternal localSensorEventQueue;

	/**
	 * The local queue from which the scheduler consumes timer events.
	 */
	private EventQueueInternal localTimerEventQueue;
	 ...
	 */
	public EventPeriodicModuleSchedulerImpl(
	...
	public void run() {
		...
		eurema.Event initialTimerEvent =
				super.createTimerEvent("Initial Timer Event");
		...
		while (super.continueRunning()) {
			try {
				...
				if (conditionMatched) {
					...
					assert eTimerEvent.getType().equals(
							EuremaEventTypeHierarchy.INSTANCE
									.getEEventType(EuremaEventType.TIMER));
					logger.debug("Processing timer event "
							+ EuremaEventTypeHierarchy.INSTANCE
									.getEEventAsString(eTimerEvent)
							+ " from queue "
							+ this.localTimerEventQueue.getName());
					...
					TimerService.INSTANCE.createTimer(periodInSec,
							this.localTimerEventQueue);
